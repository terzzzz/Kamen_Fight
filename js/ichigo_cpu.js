...
function simulateIchigoStateTransition(chi, oppLp, cpuLp, focus, airborne, faint, moveKey, move, oppAvgDmg, oppChi, currentChargeCap = 100) {
  ...
  if (moveKey.startsWith('A+')) {
    ...
    if (faint >= rules.FAINT_THRESHOLD) {
      ev = 0;
    } else if (nextCpuFaint >= rules.FAINT_THRESHOLD) {
      ev = -300;
    } else {
      ...
      if (isWindmill) {
        ...
        ev = damageSaved - (selfFaintGained * 1.5);
      } else {
        const damageSaved = oppAvgDmg * 0.70 * effectiveGuardChance;

        let chiHeadroom = Math.max(0, (rules.MAX_CHI || 16) - chi);
  ...
  if (nextOppFaint >= rules.FAINT_THRESHOLD && faint < rules.FAINT_THRESHOLD) {
    ev += 250;
